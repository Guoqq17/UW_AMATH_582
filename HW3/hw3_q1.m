...
for i=1:1:3
    for j=1:1:4
        load(strcat('cam', num2str(i), '_', num2str(j), '.mat'))
    end
end
...
thresh = 20;                            % the search threshold to find the light spot
start_point = [320,230];                % start point to search
x1=zeros(1,d1);
y1=zeros(1,d1);
for i=1:1:d1
...
%% trim the data so that they are consistent in time, and redo the experiment
% trim to same size
n = min(min(d1,d2),d3);
x1=x1(30:n);
y1=y1(30:n);
